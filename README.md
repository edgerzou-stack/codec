...
- **Code Logic First (代码逻辑优先)**: Documentation must strictly and mathematically align with the actual algorithmic implementation (e.g., pandas DataFrame filtering masks, threshold values). We reject outdated narrative descriptions or approximations in favor of absolute fidelity to the source code.
- **Hierarchical Documentation**: A complex Monorepo is effectively an amalgamation of many smaller repositories. To prevent contextual collapse, every single sub-directory maintains its own highly targeted `README.md` (complete with dynamically generated file indexes). This architecture preserves the clarity of multi-repo management while maintaining the integration benefits of a Monorepo.
